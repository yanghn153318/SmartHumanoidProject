...
c = None
minRect = None
roi_num = 0  # 전체화면(0), 상단(1), 하단(2), 하단 오른쪽(3)
...
def line_tracing(X, Y, A):
    ...
    if line_angle < -30 and X < 110:  # 라인이 왼쪽으로 비스듬하게 위치
    ...
    elif line_angle > 30 and X > 150:  # 라인이 오른쪽으로 비스듬하게 위치
        draw_str2(frame_roi, (3, 120), 'start character detection')
        # print("\n", XY_Point2[0])
...
def action(DetectedChar, CharArea, CharColor):
    ...
    if check_cardinal_point_1 == 0:
        ...
        print("\ncharacter area: " + str(CharArea) + "\n" + DetectedChar)
        print("char_color: " + CharColor)
        ...
        for i in color_num:
            ...
            if len(cnts) > 0:  # cnts에 저장된 값이 있으면(now_color와 동일한 색의 물체가 있으면) 아래코드 수행
                ...
                if Area[i] > min_area[now_color]:
                    ...
                    # 문자 인식(빨파검)
                    if (enCharDetection == 1 and enLT == 0 and enMoveCitizen == 0 and (
                            now_color == 1 or now_color == 2 or now_color == 4)):
